...
class Gripper(private val io: GripperIO) : SubsystemBase() {
    ...
    fun stop() = setVoltage(volts.zero())
    fun intakeBySensor(): Command =
        Commands.sequence(intake(), WaitUntilCommand(hasCoral), stop())
            .withName("Gripper/intakeBySensor")
    fun outtake(): Command =
        setVoltage(FAST_OUTTAKE_VOLTAGE).withName("Gripper/outTake")
    fun outtakeBySensor(): Command =
        Commands.sequence(
            outtake(),
            WaitUntilCommand(hasCoral.negate()),
            stop()
        )
    fun removeAlgae(): Command =
        setVoltage(REMOVE_ALGAE_VOLTAGE).withName("Gripper/RemoveAlgae")
    fun outtakeAlgae(): Command =
        setVoltage(OUTTAKE_VOLTAGE).withName("Gripper/outtakeAlgae")
    fun intakeAlgea(): Command =
